...
pagetable_t kvm_init_one()
{
    ...
    kvmmap_with_certain_page(newpg, UART0, UART0, PGSIZE, PTE_R | PTE_W);  // UART寄存器
    kvmmap_with_certain_page(newpg, VIRTIO0, VIRTIO0, PGSIZE, PTE_R | PTE_W);  // virtio磁盘接口
    kvmmap_with_certain_page(newpg, CLINT, CLINT, 0x10000, PTE_R | PTE_W);  // CLINT
    kvmmap_with_certain_page(newpg, PLIC, PLIC, 0x400000, PTE_R | PTE_W);  // PLIC
    kvmmap_with_certain_page(newpg, KERNBASE, KERNBASE, (uint64)etext - KERNBASE, PTE_R | PTE_X);  // 内核代码段
    kvmmap_with_certain_page(newpg, (uint64)etext, (uint64)etext, PHYSTOP - (uint64)etext, PTE_R | PTE_W);  // 内核数据段
    kvmmap_with_certain_page(newpg, TRAMPOLINE, (uint64)trampoline, PGSIZE, PTE_R | PTE_X);  // 陷阱处理 trampoline
...
{
  if (from > PLIC) // PLIC limit确保映射范围不超过PLIC（硬件设备地址）
  ...
  for (uint64 i = from; i < to; i += PGSIZE)
  {
    ...
    *pte_kernel &= ~PTE_U;   // 清除用户标志（内核访问无需PTE_U）
  }
}